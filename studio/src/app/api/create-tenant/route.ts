...
/** Scoped root so Turbopack file tracing does not treat the whole repo as bundled assets (NFT). */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const tenantPath = path.join(REPO_ROOT, baseDir, slug);
    ...
    const tenantMediaPath = path.join(REPO_ROOT, mediaBaseDir, slug);
