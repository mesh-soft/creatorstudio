...
export function SiteRenderer({ tenant, pageSlug = "home", previewLinks = false, studioMode = false }: SiteRendererProps) {
    ...
    switch (template) {
      ...
      case "hero":
        return withScrollAnchor(key, "hero", (
          <Hero tenant={tenant} block={activeBlock} blockIndex={index} variant={activeBlock?.variant || preset.hero} sectionField={sectionField} studioMode={studioMode} />
        ));
      case "profile":
...
function Section({
  ...
  return (
    <Tag id={sectionId} className={className} style={finalStyle}>
      {cssBlock && (
...
function Eyebrow({ children, editPath }: { children: ReactNode; editPath?: string }) {
  return (
...
function Text({ children, editPath }: { children: ReactNode; editPath?: string }) {
  return (
