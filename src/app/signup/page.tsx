...
export default function SignupPage() {
  ...
  const [success, setSuccess] = useState<{ tenantId: string; redirect: string } | null>(null);
  const { T, isDark, toggle } = usePageTheme();

  const passValid = password.length >= 8 && /[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(password);
  const tenantIdPreview = name.trim()
    ? name.toLowerCase().replace(/\bdr\.?\s*/g, "").replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "").replace(/^-+|-+$/g, "").slice(0, 45)
    : "";
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const res = await fetch("/api/auth/signup", {
        ...
        body: JSON.stringify({ name: name.trim(), email: email.trim(), mobile: mobile.trim(), password, role: "user" }),
      });
      const data = await res.json();
      if (!data.ok) { setError(data.error ?? "Signup failed"); return; }
      setSuccess({ tenantId: data.tenantId, redirect: data.redirect });
    ...
    return (
      <main style={{ minHeight:"100vh", background:T.shell, display:"flex", alignItems:"center", justifyContent:"center", fontFamily:FONT, color:T.text, padding:24, transition:"background .2s" }}>
        <div style={{ maxWidth:420, width:"100%", textAlign:"center", background:T.surface, borderRadius:14, border:`1px solid ${T.border}`, padding:"48px 32px", boxShadow:T.shadow, transition:"background .2s" }}>
          ...
          <h1 style={{ fontSize:20, fontWeight:800, marginBottom:8, color:T.text }}>Site Created</h1>
          <p style={{ fontSize:13, color:T.textSub, marginBottom:8 }}>Your trial site is ready. You have 5 days of free access.</p>
          <p style={{ fontSize:13, color:T.textSub, marginBottom:24 }}>Log in to continue building your site.</p>
    ...
    <main style={{ minHeight:"100vh", background:T.shell, display:"flex", alignItems:"center", justifyContent:"center", fontFamily:FONT, color:T.text, padding:24, transition:"background .2s" }}>
      <div style={{ width:"100%", maxWidth:460 }}>
        <div style={{ textAlign:"center", marginBottom:28 }}>
          <h1 style={{ margin:0, fontSize:22, fontWeight:800, letterSpacing:"-.5px", color:T.text }}>Create Your Site</h1>
          <p style={{ margin:"6px 0 0", fontSize:13, color:T.textSub }}>Get a free 5-day trial — no credit card required</p>
        </div>
        ...
        <div style={{ background:T.surface, borderRadius:14, border:`1px solid ${T.border}`, padding:"28px 28px 24px", boxShadow:T.shadow, transition:"background .2s" }}>
          <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:16 }}>
            <div>
              ...
              <input placeholder="Dr. Amit Sharma" value={name} onChange={e => setName(e.target.value)} required disabled={loading} style={inputS} onFocus={e => e.currentTarget.style.borderColor=T.accent} onBlur={e => e.currentTarget.style.borderColor=T.borderMd} />
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
              <div><label style={lbl}>Email *</label><input type="email" placeholder="amit@example.com" value={email} onChange={e => setEmail(e.target.value)} required disabled={loading} style={inputS} onFocus={e => e.currentTarget.style.borderColor=T.accent} onBlur={e => e.currentTarget.style.borderColor=T.borderMd} /></div>
            ...
            <button type="submit" disabled={loading || !passValid} style={{ padding:"11px 0", borderRadius:8, border:"none", background: loading||!passValid ? T.borderMd : T.accent, color:"#fff", fontSize:13, fontWeight:700, cursor: loading||!passValid ? "not-allowed" : "pointer", fontFamily:"inherit", marginTop:4 }}>
              {loading ? "Creating your site…" : "Start Free Trial"}
            </button>
          </form>
        </div>
        ...
        <p style={{ textAlign:"center", marginTop:8, fontSize:11, color:T.textDim }}>Want to manage multiple sites? <Link href="/signup/reseller" style={{ color:T.accent, textDecoration:"none", fontWeight:500 }}>Become a reseller →</Link></p>
